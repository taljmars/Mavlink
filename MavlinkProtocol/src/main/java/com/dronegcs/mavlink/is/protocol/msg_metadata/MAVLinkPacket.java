...
//    public static final int MAVLINK_STX = 254; //(254=0xfe, 253=0xfd) orig=254
	public static final int MAVLINK_1 = 254;
	public static final int MAVLINK_2 = 253;
    public static final int MAVLINK_STX = MAVLINK_1; //(254=0xfe, 253=0xfd)
	private static final int INCOMPAT_FLAG_SIGNED = 0x01;

	public int version = MAVLINK_1;
	...
	public int generateCRC(){
		crcObject = new CRC();
		crcObject.update_checksum(len);
		crcObject.update_checksum(seq);
		crcObject.update_checksum(sysid);
		crcObject.update_checksum(compid);
		crcObject.update_checksum(msgid);
		payload.resetIndex();
		for (int i = 0; i < payload.size(); i++) {
			crcObject.update_checksum(payload.getByte());
		}
//		return crcObject.finish_checksum(msgid);
		crcObject.finish_checksum(msgid);
		return crcObject.CRCvalue;
    }

	public static int generateCRC2(int msgid, byte[] buffer) {
//		if (this.len < 3) {
//			return -1;
//		}
		...
		crc.accumulate(buffer, 1, packetLengthWithoutCrc);

		Integer expectedCrc = CRC.MAVLINK_MESSAGE_CRCS.get(msgid);
		if (expectedCrc == null || expectedCrc == 0)
			System.out.println("Illegal message ID " + msgid);
		else {
			crc.accumulate(expectedCrc);
		}
		return crc.get();
	}
	...
	private byte[] encodePacket_mavlink2() {
		...
		buffer[i++] = (byte) (0xFF0000 & msgid);
		buffer[i++] = (byte) (0x00FF00 & msgid);
		buffer[i++] = (byte) (0x0000FF & msgid);
		for (int j = 0; j < payload.size(); j++) {
			buffer[i++] = payload.payload.get(j);
		}
		int crc = generateCRC2(msgid, buffer);
		buffer[i++] = (byte) (0xFF00 & crc);
		buffer[i++] = (byte) (0x00FF & crc);
		return buffer;
	}

    private byte[] encodePacket_mavlink1() {
		byte[] buffer = new byte[6 + len + 2];
		int i = 0;
		buffer[i++] = (byte) MAVLINK_1;
		buffer[i++] = (byte) len;
		buffer[i++] = (byte) seq;
		buffer[i++] = (byte) sysid;
		buffer[i++] = (byte) compid;
		...
		int crc = generateCRC2(msgid, buffer);
		System.out.println("Old CRC" + generateCRC());
		System.out.println("New CRC" + crc);
		buffer[i++] = (byte) (0xFF00 & crc);
		buffer[i++] = (byte) (0x00FF & crc);
//		buffer[i++] = (byte) (crcObject.getLSB());
//		buffer[i++] = (byte) (crcObject.getMSB());
		return buffer;
	}
